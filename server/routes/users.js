...
router.post('/updatePassword', function(req,res,next) {
    if('username' in req.body && 'password' in req.body && 'newPassword' in req.body){
        /* connect to database */
        req.pool.getConnection(function(error,connection){
            if (error) {
                console.log(error);
        ...
        connection.query(query,[req.body.newPassword,req.body.username,req.body.password], function(error,rows,fields) {
            if (error) {
                console.log(error);
                res.sendStatus(500);
                return;
...
router.post('/updateContactDetails', function(req,res,next) {
    if('username' in req.body && 'password' in req.body && 'email' in req.body && 'phoneNum' in req.body){
        /* connect to database */
        req.pool.getConnection(function(error,connection){
            if (error) {
                console.log(error);
        ...
        let query = `UPDATE users
                SET email = ?,
                    phoneNum = ?
                WHERE username = ?;`;
        connection.query(query,[req.body.username, req.body.email, req.body.phoneNum], function(error,rows,fields) {
            if (error) {
                console.log(error);
                res.sendStatus(500);
                return;
